...
export default function CanvasArea({
  ...
  // Line Tool (LIN) Shape & Stroke Reshape tracking refs
  ...
  // PTS Points Tool path creation state
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // 4.5. PTS Tool (Points Tool) creation logic
    if (activeTool === 'PTS') {
      const snapThreshold = 18 / zoomScale;
      // If clicking on or near the first point, close the shape into a standard drawing
      if (ptsPoints.length >= 2 && distance(coords, ptsPoints[0]) < snapThreshold) {
        const closedPts = [...ptsPoints, { ...ptsPoints[0] }];
        ...
        const newObj: VectorObject = {
          ...
          fillColor: fillToolColor && fillToolColor !== 'transparent' ? fillToolColor : 'transparent',
          opacity: 1,
          transform: { x: 0, y: 0, rotation: 0, scaleX: 1, scaleY: 1 },
      ...
      // Add clicked point to points list
    ...
    // 〰️ Line Tool (LIN) - Shape Reshape & On-Stroke Line Editing pointer down logic
    if (activeTool === 'LIN') {
      let obj: VectorObject;
      const hitObj = performHitTest(coords);
      if (hitObj && hitObj.id !== selectedObjectId) {
        setSelectedObjectId(hitObj.id);
        const hitInfo = getActiveTargetObjectInfo(hitObj.id);
        obj = hitInfo ? hitInfo.activeObj : hitObj;
      } else {
        obj = getOrPrepareActiveObject(coords);
      }

      if (!obj || obj.isLocked || obj.isHidden) return;

      // If drawing has no explicit points (e.g. geometric/circle/rect/svg), sample points around perimeter
      if (!obj.points || obj.points.length === 0) {
        const bounds = getFullObjectBounds(obj);
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (dragMode === ('lineToolDeform' as any) && activeTargetId) {
      ...
      if (obj && lineToolStartLocalRef.current && lineToolInitialPointsRef.current) {
        ...
        const deformPointList = (ptsList: Point[]) => {
          if (!ptsList || ptsList.length === 0) return ptsList;
          const count = ptsList.length;

          // 1. Distance-based smooth bell-curve displacement
          const result = ptsList.map((pt) => {
            const dist = Math.hypot(pt.x - startLocal.x, pt.y - startLocal.y);
            if (dist < R) {
              const t = dist / R;
              // Smooth cosine smoothstep kernel
              const w = Math.pow(Math.cos(t * Math.PI * 0.5), 2);
              return {
                ...pt,
          ...
          // 2. Curvature tension Laplacian smoothing across deformed zone
          if (tension > 0.05 && count > 2) {
            ...
            for (let pass = 0; pass < 2; pass++) {
              for (let i = 0; i < count; i++) {
                ...
                if (origDist < R * 1.25) {
                  ...
                  const factor = tension * 0.35 * Math.max(0, 1 - (origDist / (R * 1.25)));
                  result[i].x = Number(((1 - factor) * result[i].x + factor * midX).toFixed(2));
                  result[i].y = Number(((1 - factor) * result[i].y + factor * midY).toFixed(2));
                }
              }
            }
          }
    ...
    if (dragMode === 'contour_point' || dragMode === 'master_controller' || dragMode === 'peg_node') {
      setDragMode('none');
      historyPush();
      return;
    }
  ...
  useEffect(() => {
    ...
    // 〰️ Line Tool (LIN) - Exact On-Stroke Line & Shape Reshaper Overlay
    if (activeTool === 'LIN' && effectiveSelectedObjectId && objects[effectiveSelectedObjectId]) {
      const obj = objects[effectiveSelectedObjectId];
      if (obj && !obj.isHidden) {
        const localPivot = obj.pivots[0] || { localX: 0, localY: 0 };
        let subPathsToDraw: Point[][] = obj.subPaths && obj.subPaths.length > 0 ? obj.subPaths : [obj.points || []];
        
        // If empty points, dynamically derive contour from bounds
        if (subPathsToDraw.length === 0 || subPathsToDraw[0].length === 0) {
          const bounds = getFullObjectBounds(obj);
          const w = bounds.width > 0 ? bounds.width : 120;
          const h = bounds.height > 0 ? bounds.height : 120;
          const cx = bounds.x + w / 2;
    ...
    // Render active PTS Points Tool preview
  ...
  return (
    <div ref={containerRef} className="flex-1 bg-white relative overflow-hidden select-none">
      ...
      {/* Floating HUD Bar for PTS Points Tool */}
      {activeTool === 'PTS' && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-neutral-900/95 backdrop-blur border border-amber-500/40 px-4 py-2 rounded-2xl shadow-2xl flex items-center gap-3 animate-fade-in pointer-events-auto">
          <span className="text-[11px] font-black text-amber-400 uppercase tracking-wide flex items-center gap-1.5 font-sans">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse"></span>
            Points Tool (PTS)
          </span>
          <div className="h-4 w-px bg-neutral-800" />
          <span className="text-[10px] text-neutral-300 font-mono font-bold">
            {ptsPoints.length} {ptsPoints.length === 1 ? 'Point' : 'Points'} Placed
          </span>
          {ptsPoints.length >= 2 && (
            <>
              <button
